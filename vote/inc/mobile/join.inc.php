...
$voteuser = pdo_fetch("SELECT * FROM " . tablename($this->tablevoteuser) . " WHERE rid = :rid AND  oauth_openid = :oauth_openid ORDER BY `id` DESC", array(':rid' => $rid,':oauth_openid' => $this->oauthuser['oauth_openid']));
